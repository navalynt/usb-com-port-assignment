...
function Set-COMPortSettings {
    ...
    try {
        ...
        Set-ItemProperty -Path $deviceParamsPath -Name "FlowControl" -Value $flowControl -Type DWord -Force
        Set-ItemProperty -Path $deviceParamsPath -Name "FifoEnable" -Value $useFIFO -Type DWord -Force
        Set-ItemProperty -Path $deviceParamsPath -Name "RxFIFO" -Value $rxBuffer -Type DWord -Force
        Set-ItemProperty -Path $deviceParamsPath -Name "TxFIFO" -Value $txBuffer -Type DWord -Force
...
function Process-DeviceAssignment {
    ...
    if ($currentPort -eq $targetPort) {
        ...
        # Still apply COM settings if specified
    ...
    if (Set-COMPortAssignment -DeviceID $deviceInfo.DeviceID -CurrentCOMPort $currentPort -TargetCOMPort $targetPort -DeviceName $deviceName) {
        ...
        # Apply COM settings if specified
        if ($comSettings -ne "default") {
        ...
        # Wait a moment for device to reinitialize
        Start-Sleep -Seconds 3
...
Write-Log -Message "Script Version: 1.8" -Level INFO
